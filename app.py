...
LOGGER = logging.getLogger("whereedit_app")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Launch WhereEdit web app.")
    parser.add_argument(
        "--model-root",
        type=str,
...
def build_demo(
    ...
    def run_edit(
        ...
        edit_config = {
            ...
            "bridge_plane": bool(default_edit_config.get("bridge_plane", True)),
            "bridge_consistency": bool(default_edit_config.get("bridge_consistency", True)),
            "local_edit": bool(default_edit_config.get("local_edit", True)),
            "edit_mask_quantile": float(default_edit_config.get("edit_mask_quantile", 0.55)),
            "edit_mask_temperature": float(default_edit_config.get("edit_mask_temperature", 0.15)),
            "edit_mask_blur": int(default_edit_config.get("edit_mask_blur", 5)),
        }
    ...
    with gr.Blocks(title="WhereEdit App", css=SQUARE_PREVIEW_CSS) as demo:
        gr.Markdown("# WhereEdit App")
        gr.Markdown(
            'AutoMask-guided one-step image editing with Amplified Conditional Transport (ACT).\n'
            'Cross-attention localizes edit regions; background latents stay anchored to the source image.\n'
        ...
        with gr.Row(elem_id="editor-main-row"):
            with gr.Column(scale=5, elem_id="left-input-panel"):
                ...
                with gr.Row():
                    ...
                    bridge_lambda_input = gr.Slider(
                        label="bridge_lambda",
                        minimum=0.0,
                        maximum=1.0,
                        step=0.01,
                        value=float(default_edit_config.get("bridge_lambda", 0.5)),
                    )
