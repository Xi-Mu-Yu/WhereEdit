...
DEFAULT_EDIT_CONFIG = {
    ...
    "bridge_lambda": 0.5,
    "bridge_plane": True,
    "bridge_consistency": True,
    "local_edit": True,
    "edit_mask_quantile": 0.55,
    "edit_mask_temperature": 0.15,
    "edit_mask_blur": 5,
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run WhereEdit on PIE-Bench data and export PIE-format results.")
    ...
    parser.add_argument("--t-delta", type=float, default=None, help="Timestep gap for bridge consistency and Chord bg blend.")
    parser.add_argument("--step-scale", type=float, default=None, help="Edit update magnitude.")
    parser.add_argument(
        "--bridge-lambda",
        type=float,
        default=None,
        help="Position on the peak-bridge segment (0=source peak, 1=target peak).",
    )
    parser.add_argument(
        "--no-bridge-plane",
        dest="bridge_plane",
        action="store_false",
        default=True,
        help="Disable projection onto the source/target bridge plane.",
    )
    parser.add_argument(
        "--no-bridge-consistency",
        dest="bridge_consistency",
        action="store_false",
        default=True,
        help="Disable dual-timestep bridge direction consistency gating.",
    )
    parser.add_argument(
        "--no-local-edit",
        dest="local_edit",
        action="store_false",
        default=True,
        help="Disable insight-guided local latent editing (global transport like original Chord).",
    )
    parser.add_argument(
        "--edit-mask-quantile",
        type=float,
        default=None,
        help="Quantile for semantic edit mask (higher = smaller edit region).",
    )
    parser.add_argument(
        "--edit-mask-temperature",
        type=float,
        default=None,
        help="Sigmoid temperature for edit mask boundary sharpness.",
    )
    parser.add_argument(
        "--edit-mask-blur",
        type=int,
        default=None,
        help="Odd kernel size for spatial smoothing of the edit mask.",
    )
...
def apply_cli_overrides(args: argparse.Namespace, edit_config: Dict[str, Any], seed: Optional[int]) -> tuple[Dict[str, Any], int]:
    ...
    edit_config["bridge_plane"] = bool(args.bridge_plane)
    edit_config["bridge_consistency"] = bool(args.bridge_consistency)
    edit_config["local_edit"] = bool(args.local_edit)
    if args.edit_mask_quantile is not None:
        edit_config["edit_mask_quantile"] = float(args.edit_mask_quantile)
    if args.edit_mask_temperature is not None:
        edit_config["edit_mask_temperature"] = float(args.edit_mask_temperature)
    if args.edit_mask_blur is not None:
        edit_config["edit_mask_blur"] = int(args.edit_mask_blur)
...
def main() -> None:
    ...
    pipeline = WhereEditPipeline.from_local_weights(
